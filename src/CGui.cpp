...
void CGui::Init(void)
{
	...
	if (init) {
		...
		ChatMessage("^7ETH^332nix ^7(^3c^7)2008 - cheatersutopia.com ^3& ^7nixcoders.org");
		return;
	}
	...
	winPicker->AddWindow("Aimbot Type", CAT_AIMBOT);
	...
	winIrcChat->ChannelMessage("Status", NULL, "^oWelcome to ^dETH32nix ^oIRC chat!");
